...
public class RKF45 {
    ...
    private double nextPrintT = 0; // Следующий момент времени для печати
    private final double h_print = 0.075; // Шаг печати
    ...
    public RKF45(FuncInterf funcInterf, int NEQN, double[] y, double[] YP, double t, double TOUT, double RELERR,
        ...
        //remove this
        tvalues = new double[MAXNFE];
        evaluations = new double[NEQN][MAXNFE];
    }

    private void init() throws RKF45Exception{
        if (NEQN < 1) {
            IFLAG = 8;
 ...
 /* -----------------
    ...
    */
    ...
    private void takeAStep() throws RKF45Exception{
    ...
    /* --- successful step ---
       ...
       and evaluate derivatives there  */
        ...
        if (i < MAXNFE) {
            tvalues[i] = T;
            //System.out.printf("t = %.6f, x = [ ", T);
            for (int j = 0; j < NEQN; j++) {
                evaluations[j][i] = Y[j];
                //System.out.printf("%e ",Y[j]);
            }
            ++i;
            //System.out.println("]");
            //nextPrintT += h_print;
    ...
    public void complete() throws RKF45Exception{
        init();
    }
    ...
    public ArrayList<double[]> getYValues(){
        ArrayList<double[]> finalEval = new ArrayList<double[]>();
        for(int k = 0; k < NEQN; ++k){
            double[] row = new double[i];
            for(int j = 0; j < i; j++){
                row[j] = evaluations[k][j];
            }
            finalEval.add(row);
        }
        evaluations = null;
    ...
    public ArrayList<Double> getTVals(){
        ArrayList<Double> finalT = new ArrayList<>();
        for(int j = 0; j < i; j++){
            finalT.add(tvalues[j]);
        }
        tvalues = null;
        return finalT;
    }
